...
function M.get_plugin_list(opts)
  ...
  for plugin, module in pairs(require "onitheme.groups.plugins") do
...
function M.set_palettes(opts)
  local palette_name = "onitheme.palettes." .. opts.palette
...
function M.live_reloading(opts)
  if opts.dev then
    vim.api.nvim_create_augroup("OniTheme", { clear = true })
    vim.api.nvim_create_autocmd("BufWritePost", {
      ...
      callback = function()
        ...
        if string.match(theme, "oni") then
